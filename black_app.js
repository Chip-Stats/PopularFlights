...
var flight_track_opacity = 0.002;
...
function atmosphere(){
    ...
    var atmogeo = new THREE.SphereGeometry(radius, segments, segments);
    var atmo = new THREE.Mesh(atmogeo ,customMaterial.clone() );
    atmo.scale.multiplyScalar(1.1);
    var axis = new THREE.Vector3( 1, 0, 0 );
    var angle = -25 * Math.PI / 180;
    atmo.rotateOnAxis( axis, angle );
...
function init() {
    ...
    //scene.rotation.x = 20 * Math.PI / 180;
    ...
    earth_img = THREE.ImageUtils.loadTexture('earth_airports.png', THREE.UVMapping, function() {
        elevation_img = THREE.ImageUtils.loadTexture('elevation.jpg', THREE.UVMapping, function() {
            water_img = THREE.ImageUtils.loadTexture('water.png', THREE.UVMapping, function() {
...
function flightPointCloud() {
    ...
    for (var i = 0; i < num_points; i++) {
        ...
        colors[3 * i + 0] = 1-0.05*Math.random();
        colors[3 * i + 1] = 200/255-0.05*Math.random()+0.05*Math.random();
        colors[3 * i + 2] = 32/255-0.05*Math.random()+0.05*Math.random();
    ...
    var uniforms = {
        ...
        texture: {
            type: "t",
            value: THREE.ImageUtils.loadTexture("point.png")
        }
    };
